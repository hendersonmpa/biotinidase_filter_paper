...
summer months (Figure [[fig:biotpts]] and [[fig:galtpts]]). Environmental
...
[[fig:biotpts]] between the dashed blue lines). There is no corresponding
change in the galactosemia screen positive rate during this time
period (Figures [[fig:biotgaltpos]] between the dashed blue lines).
...
#+CAPTION[]: Time series of weekly screen positive galactosemia referrals, median weekly GALT activity and mean weekly temperature (\degree{}C).  Dashed blue lines flank a period with an elevated biotinidase deficiency screen positive rate. 
#+NAME: fig:galtpts
#+ATTR_LaTeX: :width \textwidth
#+RESULTS:
...
#+begin_src R :session *R* :results output graphics file :file ./figures/biotgaltpos.pdf :exports results :tangle yes
  biotgaltcombined <- cbind(biotweek$pos, galtweek$pos)
  biotgaltcombinedts <- xts(biotgaltcombined, biotweek$week)

  plot(biotgaltcombinedts, col = c("firebrick","steelblue4"),
       multi.panel = TRUE,
       yaxis.same = FALSE, 
       yaxis.right = FALSE,
       format.labels = "%b-%Y",
  ...
  addLegend(legend.loc = "topright",
	    legend.names = c("Biotinidase", "GALT"),
...
#+CAPTION[]: Time series of weekly screen positive biotinidase and galactosemia referrals.  Dashed blue lines flank a period with an elevated biotinidase deficiency screen positive rate. 
#+NAME: fig:biotgaltpos
#+ATTR_LaTeX: :width \textwidth
#+RESULTS:
[[file:./figures/biotgaltpos.pdf]]
